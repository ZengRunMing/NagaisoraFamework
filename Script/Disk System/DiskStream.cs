...
namespace NagaisoraFamework.Underlyingsystem
{
	...
	public struct DiskInfo
	{
		...
		public DiskInfo(string name, string description, long size, string status) : this(name, "不可用", "不可用", "不可用", description, size, status)
		{
		...
		{
			return $"{Name.TrimStart(new char[] { '\\', '.' })} {Model,-35} {InterfaceType,-10} ({Size / 1024 / 1024 / 1024,-5} GB)";
		}
	}
	...
	public class DiskStream : Stream
	{
		...
		public DiskStream(string info, int bufferSize = 512, FileAccess fileAccess = FileAccess.ReadWrite, bool IsAsync = false)
		{
			...
			BaseFileStream = new(DiskHandle, fileAccess, bufferSize, IsAsync);
		...
		public override Task<int> ReadAsync([In][Out] byte[] array, int offset, int count, CancellationToken cancellationToken)
		{
		...
		public async void AsyncRead([In][Out] byte[] array, int offset, int count)
		{
			await Task.Run(() =>
			{
		...
		[SecuritySafeCritical]
		public override void Write([In][Out] byte[] array, int offset, int count)
		{
